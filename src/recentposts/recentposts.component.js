...
class RecentPosts extends Component {
    ...
    getRecentPosts = () => {
        ...
        let allArticles = [];
        categories.map(obj =>{
            db.collection(obj).orderBy("date", "desc").limit(1)
            .get()
            .then(docs => {
                if(!docs.empty) {
                    docs.forEach((doc) => {
    ...
    render() {
        ...
        return (
            <div className='corousel-container'>
                ...
                <div>
                    <img src={recentPosts.length>0 ? recentPosts[0].recentPostsImage : ''} />
                    <p className="legend">{recentPosts.length>0 ? recentPosts[0].heading : '' }</p>
                </div>
                <div>
                    <img src={recentPosts.length>0 ? recentPosts[0].recentPostsImage : ''}  />
